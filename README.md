...
### Homepage secrets (managed by Infisical)

The `homepage-secret` Kubernetes Secret consumed by `applications/homepage`
is generated by the [Infisical Secrets Operator](platform/controllers/secrets-operator)
from the `homepage` project in the self-hosted Infisical instance. The
manifest lives at `applications/homepage/infisical.yaml` and uses
...
Two placeholder values in `applications/homepage/infisical.yaml` must be
replaced after the one-time Infisical UI setup:

| Placeholder | Where to get it |
| --- | --- |
| `REPLACE_WITH_PROJECT_ID` | Infisical UI → homepage project → Settings → Project ID |
| `REPLACE_WITH_MACHINE_IDENTITY_ID` | Infisical UI → Access → Identities → the Kubernetes-auth identity → ID |

One-time Infisical UI setup:

1. Create a project called `homepage` (or reuse an existing one).
2. Ensure an environment called `prod` exists with these six secrets at
   path `/`: `HOMEPAGE_VAR_PLATFORM_API_URL`, `HOMEPAGE_VAR_PVE_URL`,
   `HOMEPAGE_VAR_UNIFI_URL`, `HOMEPAGE_VAR_REGISTRY_URL`,
   `HOMEPAGE_VAR_UPTIME_KUMA_STATUS_URL`, `HOMEPAGE_VAR_UPTIME_KUMA_URL`.
3. Create a Machine Identity using auth method **Kubernetes** with:
   - Kubernetes host: `https://kubernetes.default.svc`
   - Allowed service account names: `infisical-homepage`
   - Allowed namespaces: `applications`
4. Add that identity to the `homepage` project with **Read** access on
   `prod`.

After filling in the two placeholders and committing, the operator will
create/refresh `homepage-secret` from Infisical every 60s. Roll the
deployment to pick up the values:

```sh
kubectl rollout restart deployment/homepage -n applications
```

No `applications/homepage/.env` file is needed anymore; the previous
manual `kubectl create secret --from-env-file=…` flow is replaced by the
operator-managed sync above.
